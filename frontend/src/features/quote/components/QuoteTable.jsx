 ...
 *
 * `readOnly` is for viewers who may look but not act — the public demo page. It
 * removes the row actions entirely rather than disabling them in place: a
 * greyed-out button still invites a click, and a demo visitor who clicks Edit and
 * gets nothing back has been told the product is broken. The reason is stated in a
 * footer note instead, where it is read once rather than repeated on every row.
 */
function QuoteTable({
  quotes = [],
  procurementType = "",
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border-default bg-surface shadow-card">
      ...
      {/* Read-only viewers get the reason in words, once, instead of a pair of
          dead controls on every row. */}
      {readOnly && (
        <div className="flex items-start gap-2 border-t border-border-default bg-surface-2 px-5 py-3">
          <svg
          ...
          <p className="text-xs leading-relaxed text-muted">
            <span className="font-medium text-content">View only.</span> Editing
            and deleting quotes is turned off in this sample workspace — nothing
            here is saved and no email is sent. Create your own workspace to
            manage quotes.
          </p>
        </div>
      )}
